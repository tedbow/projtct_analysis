...
if [[ -d "$CHECKOUT_DIR/${4#project_}s/contrib/$2" ]]; then
  ...
  if [ $rector_needed_result -eq 0 ]; then
    ...
    php -d memory_limit=1536M -d sys_temp_dir=$CHECKOUT_DIR -d upload_tmp_dir=$CHECKOUT_DIR ./vendor/bin/rector process --verbose ${4#project_}s/contrib/$2 &>  /var/lib/drupalci/workspace/phpstan-results/$1.$3.rector_out 2>> /var/lib/drupalci/workspace/phpstan-results/$1.$3.rector_stderr
