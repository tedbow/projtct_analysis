...
echo "SELECT fdfpmn.field_project_machine_name_value,
       ...
       puwr.count,
       vrl.release_nid,
       fdtv44.taxonomy_vocabulary_44_tid
...
WHERE prsv.tid = 7234
  ...
  AND fdtv44.taxonomy_vocabulary_44_tid != 13032" | drush -r /var/www/drupal.org/htdocs sql-cli --extra='--skip-column-names' | sort > /tmp/projects.tsv
