...
FROM project_release_supported_versions prsv
    ...
    LEFT JOIN project_usage_week_project puwp ON puwp.nid = prsv.nid AND puwp.timestamp = (SELECT max(timestamp) FROM project_usage_week_project) and puwp.tid = 7234
...
ORDER BY puwp.count DESC" | drush -r /var/www/drupal.org/htdocs sql-cli --extra='--skip-column-names' | sort > /tmp/projects.tsv
